...
async function testVotingWorkflow() {
    ...
    console.log("🚀 Starting Voting Summarizer Test\n");
    ...
    // Complex technical document with multiple layers
...
The convergence of these technologies suggests that future distributed systems will be increasingly heterogeneous, spanning multiple execution environments and orchestration platforms. Success will require not just technical expertise but also strong organizational practices around API design, documentation, and cross-team communication. As systems become more distributed, the human aspects of software engineering - collaboration, clear communication, and shared understanding - become even more critical to success.
    ...
    console.log('VOTING WORKFLOW TEST - Complex Technical Document');
    console.log('='.repeat(80));
    console.log('\nDocument length:', complexDocument.split(/\s+/).length, 'words\n');

    try {
        const result = await votingSummarizer.summarizeWithVoting(complexDocument);

        console.log('VOTING RESULTS');
        console.log('-'.repeat(80));
        console.log(`Total candidates generated: ${result.totalCandidates}`);
        console.log(`Processing time: ${result.processingTimeMs}ms`);
        console.log(`Average time per candidate: ${Math.round(result.processingTimeMs / result.totalCandidates)}ms`);
        console.log('\n');
        ...
        result.candidates.forEach(candidate => {
            console.log(`\nCandidate ${candidate.id} - ${candidate.approach.toUpperCase()} approach:`);
            console.log('-'.repeat(80));
            console.log(candidate.summary);
            console.log(`Word count: ${candidate.summary.split(/\s+/).length}`);
        });
        ...
        console.log('='.repeat(80));
        console.log(`Candidate ${result.selectedSummary.id} - ${result.selectedSummary.approach.toUpperCase()} approach`);
        console.log('-'.repeat(80));
        console.log(result.selectedSummary.summary);
        console.log('\n');
        console.log('SELECTION REASONING:');
        console.log('-'.repeat(80));
        console.log(result.selectionReason);
        console.log('\n');

        // Analysis
        console.log('ANALYSIS:');
        console.log('-'.repeat(80));
        const summaryLengths = result.candidates.map(c => ({
            approach: c.approach,
            words: c.summary.split(/\s+/).length
        }));
        summaryLengths.forEach(s => {
            const isWinner = s.approach === result.selectedSummary.approach ? ' ⭐ WINNER' : '';
            console.log(`${s.approach}: ${s.words} words${isWinner}`);
        });

    } catch (error) {
        console.error('Error during voting workflow:', error);
    }
}
